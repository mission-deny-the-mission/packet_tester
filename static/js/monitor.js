...
socket.on('error', (data) => {
    console.error('DEBUG: Server reported error:', data.message);
...
addBtn.addEventListener('click', () => {
    ...
    console.log('DEBUG: Add Target clicked. Target:', target);
    if (!target) return;
    
    if (monitors[target]) {
        console.warn('DEBUG: Target already monitored:', target);
        alert('This target is already being monitored.');
        return;
    }

    console.log('DEBUG: Creating monitor for:', target);
    createMonitor(target);
    targetInput.value = '';
    
    console.log('DEBUG: Emitting start_test for:', target);
    startTest(target);
});
